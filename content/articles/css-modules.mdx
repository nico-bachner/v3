...
const Component: React.FC = ({ children, variant }) => {
    return (
...
One thing to look out for is the whitespace as the argument of `.join()`. Without it, the classNames would just be concatenated into one long name that the browser cannot recognise. Adding the space separates the classNames into recognisable tokens.
